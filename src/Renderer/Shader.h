...
#define ValidProgram(x) x
	...
	*/
	...
	public:
		...
		template<ShaderType target>
		void attach(const Shader<target>& shader) { GLCall(glAttachShader(m_id, shader.m_id)); }

		template<ShaderType target>
		void detach(const Shader<target>& shader) { GLCall(glDetachShader(m_id, shader.m_id)); }
		void link();
